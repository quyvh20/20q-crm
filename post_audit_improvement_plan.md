...
> **Status (updated 2026-08-18): ✅ R0–R9.5 ARE BUILT AND SHIPPED — this is now a HISTORICAL document, not a backlog.** Verified against `main` @ `f417f46`: R0 `ab035ab` · R2 `e15c33b` · R3 `ca133c4` · R4 `3b7ad81` · R5 `02e75b7` + `a5beae5` + `21fc61f` (column drop; teardown complete) · R6 `a1afb03` · R7 `5aee49c` · R8.1 `8c812da` · R8.2 `2f44395` · R8.3 `77c5acd` · R9.1 `b534ab0` · R9.2 `13076e3` · R9.3 `5275ce9`+`a0d1327`+`2f16652` · R9.4 `35f16e9`+`2256c7f`+`8fd07d0` · R9.5 `6ca537b`. R1 is an operator checklist for the owner, not code.
>
> ⚠️ **Do not plan from the "Facts" paragraphs below.** They describe the pre-build state and several are now flatly wrong — R8.1's in particular (tasks today have 8 filter predicates including search and due ranges, keyset pagination with a 100-row clamp, a live `/tasks` page with My/Today/Overdue tabs, a 15-minute due-reminder ticker, and a registered `task_reminder` notification type). Re-read the code before writing any new plan; a stale read of this section produced a wrong recommendation on 2026-08-18.
>
> **Original framing (2026-07-29):** Source was the eight-dimension audit (plans / code debt / security / product / UX / testing / perf / ops, payments excluded) plus a six-scout implementation-fact pass, verified on `main` @ `62c21bd`.
>
> **Deploy doctrine (load-bearing, still current):** `golang-migrate` is dead on prod (dirty at v2; numbered `migrations/*.sql` do **not** run). Non-automation schema changes ship as idempotent boot guards in `cmd/server/main.go` (+ `ENABLE ROW LEVEL SECURITY` + pg_class sweep entry, never `FORCE`, + mirrored dev-only migration file). **Automation tables are the exception:** they change via the engine's `Repository.AutoMigrate` (`internal/automation/repository.go:30-66`), which runs on prod. Unique indexes use the probe-and-refuse ritual; non-zero-default columns carry a DDL `DEFAULT` (GORM omits zero values). Backend gates are `go build` + `go vet` — **never run `gofmt -w`** (tree isn't gofmt-clean). Frontend checks: `npx vitest run` (rtk breaks vitest) + `npx tsc -b`; in a worktree run `npm ci` first. Bare `C:\DEV\20q-crm\...` paths hit the MAIN checkout — check `git branch --show-current` before committing. Backend deploys run via Railway's GitHub auto-deploy on push (CI's `deploy-backend` is permanently disabled); `curl /health` reports a `build` tag to confirm which binary is live.
>
