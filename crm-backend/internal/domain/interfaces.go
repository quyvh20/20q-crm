...
type ContactRepository interface {
	...
	// BulkDeleteByIDs soft-deletes the given contacts and returns the rows it
	// ACTUALLY deleted (id + email), which is not always the ids it was asked for:
	// the statement carries a row-level write scope, so an own-scoped caller's
	// request silently skips the records they do not own. Callers that must act on
	// the deletion — erasing the lead ledger, collapsing marketing consent — have to
	// key off the returned set, never the requested one, or they act on records the
	// caller was not allowed to touch. Contact.Email is *string and may be nil.
