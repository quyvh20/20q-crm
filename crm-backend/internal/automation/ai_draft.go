...
const copilotBuildTag = "2026-08-19.1-task-status-triggers"
...
// "backend live, then frontend" (deploy.yml documents the hazard at length:
// Railway auto-deploys on push while the frontend ships from CI), but nothing
// could previously say which binary prod was running. The repo is public, so
// the tag leaks nothing.
func BuildTag() string { return copilotBuildTag }
...
func buildDraftSystemPrompt(schema *SchemaResponse) string {
	...
	b.WriteString("TRIGGER TYPES: contact_created, contact_updated, deal_stage_changed (params.to_stage = a stage id), company_updated, task_created, task_updated, task_deleted, task_status_changed (params.to_status = one of open|in_progress|completed, REQUIRED; optional params.from_status, defaults to any), <custom_slug>_created/_updated, schedule (params.cron + params.timezone), date_field (params.object, params.field, params.offset_days, params.at_time — object can be \"task\" to fire N days before/after a task's due date, e.g. an overdue reminder), email_opened (a marketing campaign email was opened), email_clicked (a link in one was clicked) — both take an optional params.campaign_id pinning one campaign.\n")
	...
	b.WriteString("CONDITION OPERATORS: eq, neq, gt, gte, lt, lte, contains, not_contains, in, not_in, is_empty, is_not_empty, starts_with, ends_with, is_true, is_false, between, last_n_days. is_empty/is_not_empty/is_true/is_false take NO value (use is_true/is_false for boolean fields and for a wait step's outcome). `between` takes exactly two values as an array, inclusive on both ends: {\"operator\":\"between\",\"value\":[1000,5000]}. `last_n_days` takes a NUMBER OF DAYS (1-3650) and is resolved against the clock when the workflow runs — always prefer it over a hardcoded date for \"in the last N days\", which would be correct only on the day you write it.\n")
