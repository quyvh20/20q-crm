...
// RegisterRoutes uses (no route-level capability guard — authorization is enforced inside RunNow).
...
func TestRunNowIntegration_WorkflowResolutionScopedToOrg(t *testing.T) {
	...
	defer engine.cancel()
	handler := NewHandler(engine, db, slog.Default(), capAllow{}, nil, nil, nil)
