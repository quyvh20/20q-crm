...
func TestValidate_MarketingSendRequiresContentID(t *testing.T) {
	...
	res := ValidateWorkflowPayload(trigger, nil, []byte(`[{"type":"send_email","id":"a1","params":{"to":"{{contact.email}}","channel":"marketing"}}]`))
	...
	res = ValidateWorkflowPayload(trigger, nil, []byte(`[{"type":"send_email","id":"a1","params":{"to":"{{contact.email}}","channel":"marketing","content_id":"`+uuid.NewString()+`"}}]`))
	assert.True(t, res.Valid, "marketing send with a valid content_id must be valid: %+v", res.Errors)

	res = ValidateWorkflowPayload(trigger, nil, []byte(`[{"type":"send_email","id":"a1","params":{"to":"{{contact.email}}","channel":"marketing","content_id":"not-a-uuid"}}]`))
	assert.False(t, res.Valid, "malformed content_id must be invalid")

	res = ValidateWorkflowPayload(trigger, nil, []byte(`[{"type":"send_email","id":"a1","params":{"to":"x@test.com"}}]`))
