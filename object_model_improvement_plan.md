...
>
> **Renumber note (2026-07-31):** the P5b file shipped as `000017b_…`, which
> golang-migrate's file source cannot parse (it requires digits immediately followed
> by `_`), so it was *silently skipped* on every `migrate up` — fresh DBs never got
> `field_permissions` from a migration. Prod was unaffected (migrations are dead
> there; the `main.go` boot guard creates the table). The pair is now
> `000071_field_permissions.{up,down}.sql`; references below use the old number.
...
- **Checklist:**
  ...
  - [x] Wire P1 validation into the write path: custom objects via `customObjUC`; system objects' non-native (`custom_fields`) subset via `orgSettingsUC.ValidateCustomFields`; native columns via adapter coercion. Display recomputed on write — system objects use each object's natural label, custom objects recompute `display_name`; **as of the P4 follow-up, custom-object titles are now resolved at *read* time from the live field defs** inside `RecordService` (`applyCustomDisplay`), so a renamed/reordered display field no longer leaves a stale title (R8 closed for custom objects; system objects keep their richer composite labels by design)
  ...
  - > **Deferred items — status after the P4 follow-up:** ✅ *relation labels* — native relation fields (e.g. a deal's contact/company) now resolve to the target's display title in the detail view (list cells stay raw by design, to avoid the per-row N+1 of §11); object_links relationships also render resolved in the `RecordRelations` panel. ✅ *custom-object linking UI* — relationships are now first-class via the `RecordRelations` panel (P4), not dropped. ✅ *R8 display titles* — custom-object titles resolve at read time from the live field defs (`applyCustomDisplay`); system objects keep their composite labels. ⏳ *still deferred:* system-object relation **clear-to-null** via the partial-update path — clearing a typed FK to NULL needs a typed-usecase/repo change, which R1 keeps out until P7 (today a relation can be set/reassigned but not cleared through the uniform endpoint).
...
- **Checklist:**
  - [x] Migration `000016`: `object_links` (both-direction indexes + unique edge) with RLS, plus `.down` (+ idempotent boot guard in `main.go`, since golang-migrate is dead on prod)
  - [x] `record_service.go` — `AddLink`/`ListLinks`/`RemoveLink` + cascade-soft-delete links on record delete (both system and custom paths); idempotent edge upsert with unique-index race recovery (`record_service_links.go`)
  - [x] `relation` field type with `target_slug` (already modelled in the P2 registry); links are many-to-many by default — **cardinality enforcement deferred** (a reversible UI/validation choice, per §16's remaining-judgment-calls note)
  - [x] Tag adapter: non-contact via `object_links` (reserved `relation_key='tags'` → `to_slug='tag'`), contacts via `contact_tags`, uniform `ListTags`/`AddTag`/`RemoveTag` API shape; bridged through one new `LinkRepository` port (no churn to the contact repo)
  - [x] Frontend relation/tag picker — `RecordRelations` panel in `ObjectDetailView` (tags chips + add/remove; relation picker = choose object → record → relation_key, reusing the uniform list endpoint), driven by new `api.ts` link/tag client
  - [x] Tests: custom↔company link, custom↔custom link, tag a deal (+ tag a contact via the legacy store), delete cascades links (system + custom); 15 link/tag unit tests + an R8 read-time-display test + 5 Docker-gated repo integration tests; 4 frontend `RecordRelations` tests + an `ObjectDetailView` relation-label test; route-shape test extended. `go build`/`go vet` clean; `npx vitest run` (582) + `npx tsc -b` clean
  - > **Routes note:** links/tags mount additively under the registry prefix — `…/registry/objects/:slug/records/:id/links` and `…/tags`, plus `DELETE /registry/links/:id` (the plan's `/api/links/:id`, kept under `/registry` until the P7 promotion). Relating/tagging is gated at sales+ (edit-level), distinct from record delete (manager+).
  - > **Tag source-of-truth guard:** the generic `AddLink` rejects `to_slug='tag'`/`relation_key='tags'` so a contact can't be tagged into `object_links` (splitting its source) — all tagging flows through the adapter, which keeps contacts on `contact_tags` (D4). Cut-over to drop `contact_tags` stays in P7.
  - > **Deferred:** auto-syncing JSONB `relation`-typed field *values* into `object_links` (plan §5.2 step 7) is not wired — relationships are managed through the explicit `…/links` API in P4; native system relations (deal→contact) stay on their FK columns untouched. Link views resolve target displays one `Get` per edge (fine for a single record's panel; the §11 batch guard targets record *list* endpoints).
  - > **Verification note:** build + vet + all backend unit tests green; 582 frontend tests + `tsc -b` green. **Ran for real against Docker Postgres 16:** all 8 repository integration tests pass (3 P2 + the 5 new `object_links`: up/down/up round-trip, FindEdge + unique-index + re-link-after-unlink, ListFrom isolation, cascade both directions, contact_tags bridge). **Plus a live HTTP end-to-end against the running backend** (`scripts/p4_e2e.py`, 13 checks): register → create custom object + company + uniform records → custom→company link with resolved "Acme Corp" label → idempotent re-link → ListLinks → self-link/tag-edge 400s → tag a company (object_links) AND a contact (contact_tags) through the identical API → untag → delete-cascade. **DB-level confirmation:** the asset→company edge shows `deleted_at` set after the record delete (R3), and `object_links` has **zero** `from_slug='contact'` rows while `contact_tags` holds the contact's tag — proving the D4 split end-to-end.
- **Definition of Done:** a custom "Project" links to a Company and to another custom
  ...
  the API. ✅ (backend + unit/integration tests + UI; live-data walkthrough pending a local stack)
- **Effort:** Large (1 week).
...
- **Files:** migration `000017b`; `record_service.go` strip/guard; small UI on the field
  editor.
- **Checklist:**
  - [x] Migration `000017b`: `field_permissions` (`hidden`/`read`/`edit`, `CHECK` constraint) with RLS, plus `.down` (+ idempotent boot guard in `main.go`, since golang-migrate is dead on prod)
  - [x] `record_service.go` — strip `hidden` fields from every read response (Get/List + the create/update echo) and reject writes touching a `hidden`/`read` field with a 403 (whole-write rejection, not a silent drop). Delegated to the same `domain.RecordAuthorizer` port as OLS via a new `FieldMask` method; owner + trusted in-process calls get the empty mask. **Automation/audit see the full record — only the API response is masked** (the trigger payload is built before masking; the audit diff only contains submitted keys, which are guaranteed unrestricted because the write was guarded first)
  - [x] Admin "Field Security" grid (`FieldSecurityManager.tsx`, Settings → Permissions, admin-only): pick an object → field × role level selectors (Edit/Read/Hidden), owner column locked-on. New `GetFieldGrid`/`SetFieldPermission` usecase + `GET`/`PUT …/registry/objects/:slug/field-permissions` routes + `api.ts` client
  - [x] Tests: 8 FLS usecase unit tests (owner/trusted bypass, hidden+read levels, default-edit, shared-OLS-cache zero-overhead, upsert+cache-bust, delete-on-edit, input validation, grid assembly) + 6 RecordService enforcement tests (strip on Get/List, reject write on hidden-update/readonly-create, strip the write echo, empty-mask no-op) + 3 Docker-gated repo tests (migration up/down/up, upsert/load/delete, `CHECK`-constraint rejection) + 3 frontend grid tests; route-shape test extended. `go build`/`go vet` clean; `npx vitest run` (588) + `npx tsc -b` clean
  - > **Keying note (deliberate deviation, mirrors P5a):** `field_permissions` is keyed by `(org_id, role_id, object_slug, field_key)`, not the plan's literal `field_id → object_fields(id)`. A custom object's fields aren't in `object_fields` until P7, so a `field_id` FK could only protect *system*-object fields — breaking "all objects equal". `(object_slug, field_key)` is exactly the key space of `UniformRecord.Fields` and `object_audit`'s diff, so the mask applies by a plain lookup on a record's own field keys. Full rationale in `migrations/000071_field_permissions.up.sql`.
  - > **Opt-in / zero-overhead:** a field with no row is fully accessible (`edit`); setting a level back to `edit` **deletes** the row, so the table holds genuine restrictions only and "no rows ⇒ empty mask ⇒ `RecordService` skips strip/guard" stays a true fast path. FLS shares the per-org OLS cache entry (loaded in the same refresh), so it adds no extra warm-path query. The two maps are cached together so a partial load failure can't leave a permissive OLS view with FLS silently emptied (which would leak sensitive fields) — on any load error the engine serves the coherent prior/empty entry.
  - > **UI placement (deviation from "on the field editor"):** the control lives under the existing admin **Settings → Permissions** tab (below the OLS grid), not on the per-field editors. Rationale: it mirrors the P5a OLS grid precedent, keeps all role/field security in one admin surface, and avoids duplicating the control across the two separate field editors (`CustomFieldManager` for system objects, `ObjectDefManager` for custom). The per-object field × role grid is still "mark a field sensitive → who can see it", not an exhaustive global matrix.
  - > **Enforcement reach (honest scope, same as P5a):** FLS lives in `RecordService`, so it bites the uniform `/api/registry/objects/:slug/records` path — the live path for custom objects and the flag-gated path for system objects. Legacy per-object routes don't strip fields; the P7 cutover brings them onto the uniform path. Documented phased design (R1), not a gap.
  - > **Verification note:** build + vet + all backend unit tests green; the 3 new repo integration tests ran for real against **Docker Postgres 16** (migration round-trip, upsert/load/delete, and the `CHECK` constraint rejecting an out-of-domain level). 588 frontend tests + `tsc -b` green. No live HTTP walkthrough: FLS stripping is only observable for a **non-owner** caller (owner bypasses by design), which a single-registering-user e2e can't produce without the org-invitation flow — so the viewer-caller behaviour is proven at the unit/integration layers where a viewer caller is constructed directly (same verification posture as P5a).
...
| **R5** | **FLS adds per-request permission lookups** → latency. | ✅ Resolved (P5b): the role→field level map is cached per org **in the same entry as the OLS map** (one refresh loads both, busted together on a permission/object-set change). When no field is restricted the map is empty, so `RecordService` skips strip/guard entirely — O(1) after warm, zero work when unused. |
...
| **R8** | **Display-name drift.** | ✅ Resolved: custom-object titles are computed at *read* time from the live field defs (`RecordService.applyCustomDisplay`), so reordering/renaming the display field can't leave a stale title; system objects use their natural composite label. (The stored `display_name` remains as a write-time fallback.) |
